...
import {
  ...
  Zap, RotateCcw, Loader2, Lightbulb, Sparkles, ChevronDown, ChevronUp, Cpu, Download
} from 'lucide-react'
import {
...
export default function FixTab() {
  ...
  const [mlComparison, setMlComparison] = useState(null) // Real ML comparison data

  const firstResult = analysisResults ? Object.values(analysisResults).find(Boolean) : null

  // Trade-off slider simulation (uses model metrics if available)
  const tradeoffData = useMemo(() => {
    if (!firstResult) return []
    const baseAccuracy = modelResults?.metrics?.accuracy || 0.85
    const basePrecision = modelResults?.metrics?.precision || 0.85
    const baseRecall = modelResults?.metrics?.recall || 0.85
    const steps = 11
    return Array.from({ length: steps }, (_, i) => {
      ...
      const accuracy = Math.round((baseAccuracy - weight * 0.12) * 100)
      const precision = Math.round((basePrecision - weight * 0.08) * 100)
      const recall = Math.round((baseRecall + weight * 0.06) * 100)

      return { label: `${Math.round(weight * 100)}%`, fairness, accuracy, precision, recall, weight }
    })
  }, [firstResult, modelResults])

  const handleDownloadModel = () => {
    if (!simResult?.modelPayload) return
    const payload = simResult.modelPayload
    const modelExport = {
      version: 'FairLens-Model-v2',
      strategyApplied: simResult.strategy,
      type: payload.ensemble ? 'Ensemble Bagged Classifier' : 'Decision Tree Classifier',
      trees: payload.numTrees || 1,
      accuracy: payload.metrics?.accuracy,
      f1Score: payload.metrics?.f1,
      fairness: payload.fairnessMetrics,
      topFeatures: payload.featureImportance?.slice(0, 5),
      classifiers: payload.classifiers || [payload.classifier]
    }
    const blob = new Blob([JSON.stringify(modelExport, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `fairlens-debiased-model-${selectedStrategy || 'export'}.json`
    a.click()
  }
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data.success && data.comparison) {
          ...
          // Also create a simResult compatible with the existing UI
          const origDI = comp.baseline.modelDI || firstResult.disparateImpact
          const newDI = comp.remediated.modelDI || origDI
          const origScore = origDI >= 0.8 ? 'PASS' : 'FAIL'
          const newScore = newDI >= 0.8 ? 'PASS' : 'FAIL'

          // Merge groups from ML results with statistical groups
          const simGroups = firstResult.groups.map(g => {
            const remGroup = comp.remediated.groups?.find(rg => rg.name === g.name)
            return {
              ...g,
              newRate: remGroup ? remGroup.predRate : g.rate
            }
          })
            ...
            const result = {
              ...
              origDI,
              newDI,
              origSP: comp.baseline.statisticalParity || firstResult.statisticalParity,
              newSP: comp.remediated.statisticalParity || 0,
              origScore,
              newScore,
              improvement: comp.improvement.diChangePercent,
              mlPowered: true,
              modelPayload: comp.remediated
            }
    ...
    <div className="fade-in-up">
      ...
      {simResult && !applying && (
        <>
          ...
          {mlComparison && (
            <div className="card fade-in-up" style={{ marginTop: '24px', border: '1px solid rgba(56,178,172,0.2)' }}>
              <div className="card-header" style={{ background: 'rgba(56,178,172,0.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{
              ...
              <div className="card-body">
                  ...
                  {[
                    ...
                    return (
                      <div key={m.label} style={{
                        ...
                        <p style={{ fontSize: '0.6875rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '6px' }}>
                          {m.label}
                        </p>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                          <span style={{ fontSize: '0.8125rem', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>
                            {(m.before * 100).toFixed(1)}
                          </span>
                          <ArrowRight size={12} style={{ color: 'var(--text-muted)' }} />
                          <span style={{ fontSize: '1rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: m.color }}>
                            {(m.after * 100).toFixed(1)}%
                          </span>
                        </div>
                        <span style={{
                ...
                {mlComparison.removedProxies?.length > 0 && (
                  <div style={{
