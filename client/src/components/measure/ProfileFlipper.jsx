...
export default function ProfileFlipper({ rows, columns, sensitiveAttrs, targetColumn }) {
  ...
  const doFlip = async () => {
    ...
    if (modelResults) {
      try {
        const flippedRow = { ...currentRow, [flipAttr]: flipValue }
        const resp = await fetch(API_URL + '/analysis/predict', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ row: currentRow, flippedRow })
        })
        ...
        if (resp.ok) {
          ...
          if (data.success) {
            setResult({
              ...
              confidence: 1,
              sampleSize: modelResults.metrics?.testSize || rows.length,
              positiveRate: data.flipped.prediction === 1 ? 1 : 0,
              mlPowered: true
            })
            setMlPowered(true)
            setAnimating(false)
        ...
        console.warn('ML predict failed, falling back to statistical:', err.message)
      }
    }
          ...
          <div className="fade-in-up">
            ...
            {mlPowered && (
              <div style={{
                ...
                fontSize: '0.625rem', color: 'var(--accent-teal)', fontWeight: 600
              }}>
                <Cpu size={10} /> Prediction from trained ML model
              </div>
            )}
          </div>
        )}
