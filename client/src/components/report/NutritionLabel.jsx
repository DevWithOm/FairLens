...
export default function NutritionLabel({ datasetName, rows, columns, sensitiveAttrs, targetColumn, analysisResults, remediationResults, modelResults }) {
  const { t } = useData()
          ...
          <div style={{ padding: '16px 20px' }}>
            ...
            {modelResults && (
              <>
                <Divider />
                <div style={{ fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '8px' }}>
                  {t('ML PIPELINE PERFORMANCE')}
                </div>
                <MetricRow label={t('Model Engine')} value={modelResults.ensemble ? `Ensemble (${modelResults.numTrees} trees)` : 'Decision Tree'} status="none" />
                <MetricRow label={t('Accuracy')} value={`${(modelResults.metrics.accuracy * 100).toFixed(1)}%`} status={modelResults.metrics.accuracy >= 0.7 ? 'pass' : 'warn'} />
                <MetricRow label={t('F1 Score')} value={`${(modelResults.metrics.f1 * 100).toFixed(1)}%`} status={modelResults.metrics.f1 >= 0.7 ? 'pass' : 'warn'} />
                {modelResults.featureImportance && modelResults.featureImportance.length > 0 && (
                  <div style={{ padding: '4px 0', fontSize: '0.8125rem' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>{t('Top Drivers:')}</span>{' '}
                    <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>
                      {modelResults.featureImportance.slice(0, 2).map(f => f.feature).join(', ')}
                    </span>
                  </div>
                )}
              </>
...
function MetricRow({ label, value, status }) {
  const color = status === 'pass' ? '#48BB78' : status === 'warn' ? '#ECC94B' : status === 'fail' ? '#FC8181' : 'var(--text-primary)'
  return (
    <div style={{
      display: 'flex',
      justifyContent: 'space-between',
